...
class Viewer(core.Worker): 
    ...
    def __init__(self, data, events): 
        ...
        # This makes the right column visible across all tabs.
        # -----------------------------------------------------------------
        ...
        # Build the 'Buffers' tab content (unchanged)
        self._build_buffers_tab()

        # -----------------------------------------------------------------
        # Persistent right column: always visible (Status, Piezos, Profile size, ROI preview)
        # -----------------------------------------------------------------
        ...
        pid_frame = ttk.LabelFrame(cfg_wrap, text="PID OPD", padding=10)
        ...
        self.pid_p = tk.DoubleVar(value=_read_float('Servo.pid_P', self.data['Servo.PID_OPD'][0]))
        self.pid_i = tk.DoubleVar(value=_read_float('Servo.pid_I', self.data['Servo.PID_OPD'][1]))
        self.pid_d = tk.DoubleVar(value=_read_float('Servo.pid_D', self.data['Servo.PID_OPD'][2]))
        ...
        # Parent is self.root so the toolbar is always visible.
        # ---------------------------------------------------------------------
        ...
        # (Optional) Clear ROI button kept commented as in your code
    ...
    def _apply_pid(self): 
        try: 
            self.data['Servo.PID_OPD'][:3] = np.array(
    ...
    def _build_piezos_controls(self, parent):
        ...
        # Lire les valeurs initiales si présentes
        try:
        ...
        def _col(label, var, col):
            ...
            # --- NEW: barre de boutons sous la règle
        ...
        # Initial push vers le buffer partagé
    ...
    def _format_preview(self, value, max_elems=12, head=6, tail=6, precision=3):
        ...
        def _is_scalar(x):
            try:
                ...
                pass
            # size==1 container often used in self.data for scalars
            try:
                arr = _np.array(x)
                return arr.size == 1
            except Exception:
        ...
        if _is_scalar(value):
            try:
                arr = _np.array(value).reshape(-1)
                v = arr[0]
                return f"{float(v):.{precision}f}" if _np.issubdtype(arr.dtype, _np.number) else str(v)
            except Exception:
                return str(value)
        ...
        flat = arr.ravel()
        n = flat.size
        # dtype/shape shown elsewhere; here we only care about the preview string
        # number formatting
        def _fmt(x):
            try:
                return f"{float(x):.{precision}f}" if _np.issubdtype(flat.dtype, _np.number) else str(x)
            except Exception:
                return str(x)
    ...
    def reset_tiptilt(self):
        self.data['IRCamera.hprofile_target_angle_diff'][0] = self.data['IRCamera.hprofile_angle_diff'][0]
        self.data['IRCamera.vprofile_target_angle_diff'][0] = self.data['IRCamera.vprofile_angle_diff'][0]
